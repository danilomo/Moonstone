    ...
    (column #:spacing 4
      ...
      (row #:spacing 8 #:vertical-alignment 'center #:fill-max-width #t
        ...
        (slider #:value cur
                #:min mn #:max mx
                #:modifier '(("weight" 1.0))
                #:on-change (lambda (v) (state-set! in-s (format-number v dec))))
  ...
  (let ((checked (string=? (state-ref in-s) "1")))
    (column #:spacing 0
      (switch
        ...
        #:on-change (lambda (v) (state-set! in-s (if (= v 1) "1" "0")))
        (metric-field-label m in-s default-s)))))

(define (make-star-button in-s star-val filled)
  (button #:style 'text
    #:on-click (lambda () (state-set! in-s (number->string star-val)))
        ...
        (append
          ...
          (map (lambda (opt)
                 (radio-button
                   #:selected in-s
                   #:value opt
                   #:on-select (lambda () (state-set! in-s opt))
                   (text #:value opt)))
               options))))))
